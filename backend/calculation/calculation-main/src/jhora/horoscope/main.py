...
"""
# Suppress strict static type checks for this dynamic, legacy-heavy module which
# uses runtime-populated resources and lacks type stubs for external modules.
...
class Horoscope():  
    ...
    def get_calendar_information(self):#, language='en'):
        ...
        # Weekday (civil) previously used drik.vaara(jd) which could flip around midnight.
        # Civil weekday: Python Monday=0..Sunday=6. Panchanga list expects Sunday=0..Saturday=6.
        # Map: Sunday(6)->0, Monday(0)->1, Tuesday(1)->2 ... Saturday(5)->6.
        from datetime import datetime as _dt
        _civil_weekday = _dt(self.Date.year, self.Date.month, self.Date.day).weekday()  # Monday=0
        _panchanga_weekday = (_civil_weekday + 1) % 7
        try:
            calendar_info[cal_key_list['vaaram_str']] = utils.DAYS_LIST[_panchanga_weekday]
        except Exception:
            # Fallback to original method if resource lists not yet loaded
        ...
        # Tithi string formatting: show start then end
        _tithi_start_str = utils.to_dms(_tithi[1])
        _tithi_end_str = utils.to_dms(_tithi[2])
        calendar_info[cal_key_list['tithi_str']] = (
            f"{utils.PAKSHA_LIST[_paksha]} {utils.TITHI_LIST[_tithi[0]-1]} ({utils.TITHI_DEITIES[_tithi[0]-1]}) "
            f"starts at {_tithi_start_str} ends at {_tithi_end_str} ("+"{0:.2f}".format(frac_left)+"% "
            f"{cal_key_list['balance_str']} )"
        )
        ...
        _nak_start_str = utils.to_dms(nak[2])
        _nak_end_str = utils.to_dms(nak[3])
        calendar_info[cal_key_list['nakshatra_str']] = (
            f"{utils.NAKSHATRA_LIST[nak[0]-1]} ("+utils.PLANET_SHORT_NAMES[utils.nakshathra_lord(nak[0])]+") "
            f"{cal_key_list['paadham_str']}{nak[1]} starts at {_nak_start_str} ends at {_nak_end_str} ("
            +"{0:.2f}".format(frac_left)+f"% {cal_key_list['balance_str']} )"
        )
        ...
        _yoga_start_str = utils.to_dms(yogam[1])
        _yoga_end_str = utils.to_dms(yogam[2])
        calendar_info[cal_key_list['yogam_str']] = (
            f"{utils.YOGAM_LIST[yogam[0]-1]} {yoga_lord}starts at {_yoga_start_str} ends at {_yoga_end_str} ("
            +"{0:.2f}".format(yogam[3]*100)+f"% {cal_key_list['balance_str']} )"
        )
        ...
        _kar_start_str = utils.to_dms(karanam[1])
        _kar_end_str = utils.to_dms(karanam[2])
        calendar_info[cal_key_list['karanam_str']] = (
            f"{utils.KARANA_LIST[karanam[0]-1]} ({karana_lord}) starts at {_kar_start_str} ends at {_kar_end_str} ("
            +"{0:.2f}".format(frac_left)+f"% {cal_key_list['balance_str']} )"
        )
    ...
    def get_horoscope_information_for_chart(self,chart_index=0,chart_method=1,divisional_chart_factor=None,
        ...
        horoscope_info['ascendant_data'] = ascendant_navamsa
    ...
    def _get_vimsottari_dhasa_bhukthi(self,dob,tob,place,**kwargs):#,divisional_chart_factor=1):
        ...
        # Keep the balance on the Horoscope instance and return (balance, periods)
        ...
        for i in range(len(db)):
            ...
            dhasa_bhukti_info.append((utils.PLANET_NAMES[dhasa_lord] + '-' + utils.PLANET_NAMES[bukthi_lord], bukthi_start))
        return self._vimsottari_balance, dhasa_bhukti_info

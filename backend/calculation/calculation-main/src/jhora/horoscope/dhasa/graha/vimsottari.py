...
"""
Calculates Vimshottari (=120) Dasha-bhukthi-antara-sukshma-prana with 5 levels support
"""
...
def _vimsottari_pratyantara(maha_lord, bhukti_lord, antara_lord, start_date):
    """Compute all Pratyantaradasas (3rd level) from given antara's start date.
    Formula: (Maha × Bhukti × Antara × Pratyan) / 120³ years"""
    lord = antara_lord
    retval = Dict()
    for i in range(9):
        retval[lord] = start_date
        factor = vimsottari_dict[lord] * (vimsottari_dict[maha_lord] / human_life_span_for_vimsottari_dhasa)
        factor *= (vimsottari_dict[bhukti_lord] / human_life_span_for_vimsottari_dhasa)
        factor *= (vimsottari_dict[antara_lord] / human_life_span_for_vimsottari_dhasa)
        start_date += factor * year_duration
        lord = vimsottari_next_adhipati(lord)

    return retval

def _vimsottari_sookshma(maha_lord, bhukti_lord, antara_lord, pratyan_lord, start_date):
    """Compute all Sookshma dasas (4th level) from given pratyantara's start date.
    Formula: (Maha × Bhukti × Antara × Pratyan × Sookshma) / 120⁴ years"""
    lord = pratyan_lord
    retval = Dict()
    for i in range(9):
        retval[lord] = start_date
        ...
        factor *= (vimsottari_dict[bhukti_lord] / human_life_span_for_vimsottari_dhasa)
        factor *= (vimsottari_dict[antara_lord] / human_life_span_for_vimsottari_dhasa)
        factor *= (vimsottari_dict[pratyan_lord] / human_life_span_for_vimsottari_dhasa)
        start_date += factor * year_duration
        lord = vimsottari_next_adhipati(lord)

    return retval

def _vimsottari_prana(maha_lord, bhukti_lord, antara_lord, pratyan_lord, sookshma_lord, start_date):
    """Compute all Prana dasas (5th level) from given sookshma's start date.
    Formula: (Maha × Bhukti × Antara × Pratyan × Sookshma × Prana) / 120⁵ years"""
    lord = sookshma_lord
    retval = Dict()
    for i in range(9):
        retval[lord] = start_date
        ...
        factor *= (vimsottari_dict[antara_lord] / human_life_span_for_vimsottari_dhasa)
        factor *= (vimsottari_dict[pratyan_lord] / human_life_span_for_vimsottari_dhasa)
        factor *= (vimsottari_dict[sookshma_lord] / human_life_span_for_vimsottari_dhasa)
        start_date += factor * year_duration
        lord = vimsottari_next_adhipati(lord)
...
def get_vimsottari_dhasa_levels(jd, place, depth=2, **kwargs):
    """
    Get Vimsottari Dasha with configurable depth (1-5 levels)
    @param depth: 1=MD only, 2=MD+AD, 3=MD+AD+PD, 4=MD+AD+PD+SD, 5=MD+AD+PD+SD+PAD
    @return: Nested dict structure with requested depth
    """
    if depth < 1 or depth > 5:
        raise ValueError("Depth must be between 1 and 5")
    
    # Get mahadashas
    dashas = vimsottari_mahadasa(jd, place, **kwargs)
    result = []
    
    for maha_lord, maha_start in dashas.items():
        y, m, d, h = utils.jd_to_gregorian(maha_start)
        maha_entry = {
            'lord': maha_lord,
            'start': '%04d-%02d-%02d' %(y,m,d)+' '+utils.to_dms(h,as_string=True),
        }
        
        if depth >= 2:
            # Add Antardashas
            bhuktis = _vimsottari_bhukti(maha_lord, maha_start)
            maha_entry['antardasha'] = []
            ...
            for bhukti_lord, bhukti_start in bhuktis.items():
                ...
                if depth >= 3:
                    # Add Pratyantardashas
                    antaras = _vimsottari_antara(maha_lord, bhukti_lord, bhukti_start)
                    bhukti_entry['pratyantara'] = []
                    
                    for antara_lord, antara_start in antaras.items():
                        ...
                        if depth >= 4:
                            # Add Sookshmas
                            pratyantaras = _vimsottari_pratyantara(maha_lord, bhukti_lord, antara_lord, antara_start)
                            antara_entry['sookshma'] = []
                            
                            for pratyan_lord, pratyan_start in pratyantaras.items():
                                y, m, d, h = utils.jd_to_gregorian(pratyan_start)
                                pratyan_entry = {
                                    'lord': pratyan_lord,
                                    'start': '%04d-%02d-%02d' %(y,m,d)+' '+utils.to_dms(h,as_string=True),
                                }
                                ...
                                if depth >= 5:
                                    # Add Pranas
                                    sookshmas = _vimsottari_sookshma(maha_lord, bhukti_lord, antara_lord, pratyan_lord, pratyan_start)
                                    pratyan_entry['prana'] = []
                                    
                                    for sookshma_lord, sookshma_start in sookshmas.items():
                                        y, m, d, h = utils.jd_to_gregorian(sookshma_start)
                                        sookshma_entry = {
                                            'lord': sookshma_lord,
                                            'start': '%04d-%02d-%02d' %(y,m,d)+' '+utils.to_dms(h,as_string=True),
                                        }
                                        pratyan_entry['prana'].append(sookshma_entry)
                                
                                antara_entry['sookshma'].append(pratyan_entry)
                        
                        bhukti_entry['pratyantara'].append(antara_entry)
                
                maha_entry['antardasha'].append(bhukti_entry)

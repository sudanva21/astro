#!/usr/bin/env python
# pyright: ignore[reportArgumentType,reportCallIssue,reportGeneralTypeIssues]
# -*- coding: UTF-8 -*-
...
# Unified strength thresholds for reuse across yoga checks
try:
    _STRENGTH_WEAK = const._ENEMY
    _STRENGTH_OK = const._NEUTRAL_SAMAM
    _STRENGTH_GOOD = const._FRIEND
    _STRENGTH_STRONG = const._EXALTED_UCCHAM
except Exception:
    _STRENGTH_WEAK = -1; _STRENGTH_OK = 0; _STRENGTH_GOOD = 1; _STRENGTH_STRONG = 2
...
def get_yoga_details(jd,place,divisional_chart_factor=1,language='en'):
    ...
    for yoga_function,details in msgs.items():
        # Resolve function safely instead of eval for security/performance.
        func_name = yoga_function + '_from_planet_positions'
        fn = globals().get(func_name)
        if not callable(fn):
            continue
        try:
            yoga_exists = fn(planet_positions)
        except Exception:
            yoga_exists = False
        if yoga_exists:
            # Insert chart identifier at index 0 (mutating copy of list from msgs json)
            try:
...
The original implementation performs many nested index lookups directly on
`const.house_strengths_of_planets` and dictionaries derived from runtime
calculations. Pyright / static analyzers flag these as potential type issues
because upstream libraries may return `numpy.ndarray`, unions or untyped
containers. To keep runtime behaviour unchanged while silencing false positive
diagnostics and guarding against unexpected types, we introduce thin wrappers
that coerce inputs to `int` safely and fall back gracefully on failure.
"""

def _planet_house(p_to_h: Dict[Any, Any], planet: int) -> int:
    try:
        h = p_to_h[planet]
        if isinstance(h, (list, tuple)) and h:
            h = h[0]
        return int(h)  # type: ignore[arg-type]
    except Exception:
        return 0

def _as_int(val: Any) -> int:
    """Best-effort int conversion used to placate static checkers; returns 0 on failure."""
    try:
        return int(val)  # type: ignore[arg-type]
    except Exception:
        return 0

def _hs(planet: int, house_index: int) -> int:
    """Return house strength value as int (0 on failure)."""
    try:
        row = const.house_strengths_of_planets[planet]  # type: ignore[index]
        val = row[house_index]  # type: ignore[index]
...
def kemadruma_yoga_from_planet_positions(planet_positions):
    ...
    # Cancellation 1: Any planet (other than Sun) in 2nd or 12th from Moon actually defeats pure Kemadruma.
    cancellation_flankers = any([p in planets_from_moon for p in range(2,9)])
    # Cancellation 2: Any planet (other than Moon) in a kendra from Moon (1,4,7,10 from Moon)
    kendras_from_moon = [((h+moon_house-1)%12) for h in [1,4,7,10]]
    planets_kendra_moon = [p for h in kendras_from_moon for p,h1 in p_to_h.items() if h==h1]
    cancellation_kendra = any([p for p in planets_kendra_moon if p not in [0,1]])
    kemadruma = ky1 and ky2 and not (cancellation_flankers or cancellation_kendra)
    return kemadruma
...
def adhi_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    target_houses = [h_to_p[(p_to_h[1]+mh-1)%12] for mh in [6,7,8]]
    # Flatten planet strings
    planets_there = set(sum([seg.split('/') for seg in target_houses if seg], []))
    # Benefics: Jupiter(4), Venus(5); Mercury(3) counts only if not with malefic (6 Saturn,2 Mars,0 Sun sometimes,7 Rahu,8 Ketu?)
    # We'll consider natural malefics list for contamination.
    malefic_tags = set([str(p) for p in const.natural_malefics])
    mercury_is_benefic = '3' in planets_there and not (planets_there & malefic_tags)
    benefic_presence = {'4': '4' in planets_there, '5': '5' in planets_there, '3': mercury_is_benefic}
    # Require at least two benefics spanning those houses for stronger Adhi interpretation
    adhi_yoga = sum(1 for k,v in benefic_presence.items() if v) >= 2
    return adhi_yoga
...
def gaja_kesari_yoga_from_planet_positions(planet_positions):
    ...
    """ Gaja-Kesari Yoga: If (1) Jupiter is in a quadrant from Moon, (2) a benefic planet
        ...
        enemy’s house, """
    # Classical (simplified) criteria we implement:
    # 1. Jupiter (index 4) in kendra (quadrant) from Moon (index 1).
    # 2. A benefic (natural benefics list) either conjoins Jupiter or aspects Jupiter's house.
    # 3. Jupiter not in enemy/debilitated sign (strength matrix value > _ENEMY).
    # 4. Jupiter not combust.
    # If data missing or any failure, fall back gracefully to False.
    try:
        p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
        jupiter_house = p_to_h[4]
        moon_house = p_to_h[1]
        chk1 = jupiter_house in quadrants_of_the_house(moon_house)
        if not chk1:
            return False
        conj_planets = [p for p in h_to_p[jupiter_house].split('/') if p not in ('', const._ascendant_symbol)]
        aspected_planets = house.aspected_planets_of_the_raasi(h_to_p, jupiter_house)
        benefics = set(str(b) for b in const.natural_benefics)
        chk2 = any(p in benefics for p in conj_planets) or any(p in benefics for p in aspected_planets)
        if not chk2:
            return False
        strength = _hs(4, jupiter_house)
        chk3 = strength >= const._NEUTRAL_SAMAM
        if not chk3:
            return False
        try:
...
def kaahala_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    fourth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+3)%12))
    lagna_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,asc_house))
    ...
    if not ky1:
        return False
    ky2 = _hs(lagna_lord, asc_house) > const._NEUTRAL_SAMAM
...
def chaamara_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    lagna_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,asc_house))
    lagna_house = _planet_house(p_to_h, lagna_lord)
    cy = [[(str(nb) in h_to_p[a]) for a in [(asc_house+6)%12,(asc_house+8)%12,(asc_house+9)%12]] for nb in const.natural_benefics]
    cy1 = sum(sum(cy,[])) > 1
    if cy1:
        return True
    cy2 = _hs(lagna_lord, lagna_house) >= const._EXALTED_UCCHAM and str(lagna_lord) in house.graha_drishti_of_the_planet(h_to_p, 4)
...
def sankha_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    fifth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+4)%12))
    sixth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+5)%12))
    lagna_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,asc_house))
    ...
    ky3 = _hs(lagna_lord, asc_house) > const._NEUTRAL_SAMAM
    ninth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+8)%12))
    ky4 = _hs(ninth_lord, _planet_house(p_to_h, ninth_lord)) > const._NEUTRAL_SAMAM
    ky5 = _planet_house(p_to_h, lagna_lord) == _planet_house(p_to_h, ninth_lord) and _planet_house(p_to_h, lagna_lord) in const.movable_signs
...
def bheri_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    ninth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+8)%12))
    by1 = _hs(ninth_lord, _planet_house(p_to_h, ninth_lord)) > const._NEUTRAL_SAMAM
    by2 = all([ any([str(p_to_h[p]) in str(hp) for hp in [asc_house, (asc_house+1)%12, (asc_house+6)%12, (asc_house+11)%12]]) for p in [*range(9)] ])
    by3_flags = [h in quadrants_of_the_house(asc_house) for h in [p_to_h[4], p_to_h[5], asc_house]]
    return bool(by1 and (by2 or any(by3_flags)))
def mridanga_yoga_from_planet_positions(planet_positions):
    ...
    my1 = any([any([p_to_h[p]==h and _hs(p, h)>const._FRIEND for h in quadrants_and_trines]) for p in range(9)])
    lagna_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,asc_house))
    my2 = _hs(lagna_lord, _planet_house(p_to_h, lagna_lord)) > const._FRIEND
    return my1 and my2
def sreenaatha_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    seventh_lord = _owner(planet_positions,(asc_house+6)%12)
    ninth_lord = _owner(planet_positions,(asc_house+8)%12)
    # BUGFIX: tenth_lord previously (asc_house+8)%12 (same as ninth) -> should be (asc_house+9)%12
    tenth_lord = _owner(planet_positions,(asc_house+9)%12)
    sy1 = _hs(seventh_lord, (asc_house+9)%12)>const._FRIEND
    sy2 = _planet_house(p_to_h, ninth_lord) == _planet_house(p_to_h, tenth_lord)
    return sy1 and sy2
def matsya_yoga_from_planet_positions(planet_positions):
    ...
    my1_flags = [any(str(p) in str(nb) for nb in const.natural_benefics) for p in ph]
    if not any(my1_flags):
        return False
    ph = sum([h_to_p[h].split('/') for h in [(asc_house+3)%12,(asc_house+7)%12]],[])
    ph = [p for p in ph if p != '']
    my3_flags = [any(str(p) in str(nb) for nb in const.natural_malefics) for p in ph]
    my2 = h_to_p[(asc_house+4)%12] != ''
    return any(my1_flags) and my2 and any(my3_flags)
def koorma_yoga_from_planet_positions(planet_positions):
    ...
    if not my1:
        return False
    my1 = all([_hs(int(p), _planet_house(p_to_h, int(p))) > const._NEUTRAL_SAMAM for p in ph if p!=const._ascendant_symbol])
    ...
    if my2:
        my2 = all([_hs(int(p), _planet_house(p_to_h, int(p))) > const._NEUTRAL_SAMAM for p in ph  if p!=const._ascendant_symbol])
    return my1 and my2
def khadga_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    second_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+1)%12))
    ninth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+8)%12))
    lagna_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,asc_house))
    ky1 = (_planet_house(p_to_h, second_lord) == (asc_house+8)%12) and (_planet_house(p_to_h, ninth_lord) == (asc_house+1)%12)
    quadrants_and_trines = house.quadrants_of_the_raasi(asc_house)+house.trines_of_the_raasi(asc_house)
    ky2 = _planet_house(p_to_h, lagna_lord) in quadrants_and_trines
    return ky1 and ky2
...
def kalaanidhi_yoga_from_planet_positions(planet_positions):
    ...
    if not ky1:
        return False
    # Need BOTH Mercury(3) and Venus(5) involvement either by conjunction or aspect
    conj_set = set(h_to_p[p_to_h[4]].split('/')) if h_to_p[p_to_h[4]] else set()
    aspect_set = set(house.aspected_planets_of_the_raasi(h_to_p, p_to_h[4]))
    involved = {pid for pid in ['3','5'] if (pid in conj_set or pid in aspect_set)}
    ky2 = len(involved) == 2
    return ky1 and ky2

# Helper to safely fetch house owner as int
def _owner(planet_positions, house_index: int) -> int:
    try:
        return _as_int(house.house_owner_from_planet_positions(planet_positions, house_index))
    except Exception:
        return -1
def kalpadruma_yoga_from_planet_positions(planet_positions):
    ...
    # Comment this for actual """
    lagna_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,asc_house))
    depositor_1 = _as_int(house.house_owner_from_planet_positions(planet_positions,_planet_house(p_to_h, lagna_lord)))
    depositor_2 = _as_int(house.house_owner_from_planet_positions(planet_positions,_planet_house(p_to_h, depositor_1)))
    p_to_h_navamsa = globals().get('p_to_h_navamsa', {})
    if p_to_h_navamsa:
        try:
            depositor_3 = _as_int(house.house_owner_from_planet_positions(planet_positions,_planet_house(p_to_h_navamsa, depositor_1)))
        except Exception:
            depositor_3 = depositor_2
    else:
        depositor_3 = depositor_2
    all_four_planets = list(map(_as_int,[lagna_lord,depositor_1,depositor_2,depositor_3]))
    ky1 = []
    quadrants_and_trines_of_rasi = house.quadrants_of_the_raasi(asc_house)+house.trines_of_the_raasi(asc_house)
    asc_house_navamsa = p_to_h_navamsa.get(const._ascendant_symbol, asc_house)
    ...
    for p in all_four_planets:
        kyp = _hs(p, _planet_house(p_to_h, p)) > const._FRIEND
        kyp = kyp or _planet_house(p_to_h, p) in quadrants_and_trines_of_rasi
        ky1.append(kyp)
    ky2 = []
    for p in all_four_planets:
        kyp = _hs(p, _planet_house(p_to_h_navamsa, p)) > const._FRIEND
        kyp = kyp or _planet_house(p_to_h_navamsa, p) in quadrants_and_trines_of_navamsa
        ky2.append(kyp)
...
def hari_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    second_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+1)%12))
    lord_house = _planet_house(p_to_h, second_lord)
...
def hara_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    seventh_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+6)%12))
    lord_house = _planet_house(p_to_h, seventh_lord)
...
def brahma_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    lagna_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,asc_house))
    lord_house = _planet_house(p_to_h, lagna_lord)
    ...
    ninth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+8)%12))
    eleventh_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+10)%12))
    alt_lagna_lord = lagna_lord  # reuse
    tenth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+9)%12))
    ly2 = p_to_h[4] in quadrants_of_the_house(_planet_house(p_to_h, ninth_lord))
    ly3 = p_to_h[5] in quadrants_of_the_house(_planet_house(p_to_h, eleventh_lord))
    ly4_1 = p_to_h[3] in quadrants_of_the_house(_planet_house(p_to_h, alt_lagna_lord))
    ly4_2 = p_to_h[3] in quadrants_of_the_house(_planet_house(p_to_h, tenth_lord))
    # BUGFIX: duplicated ly4_1 in OR; should combine ly4_1 or ly4_2
    return ly1 or (ly2 and ly3 and (ly4_1 or ly4_2))
def vishnu_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    ninth_lord_in_rasi = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+8)%12)) 
    tenth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+9)%12))
    vy1 = _planet_house(p_to_h, ninth_lord_in_rasi)==(asc_house+1)%12 and _planet_house(p_to_h, tenth_lord)==(asc_house+1)%12
    if not vy1:
        return False
    navamsa_map = globals().get('p_to_h_navamsa', {})
    if ninth_lord_in_rasi in navamsa_map:
        lord_of_ninth_in_navamsa = _as_int(house.house_owner_from_planet_positions(planet_positions, navamsa_map[ninth_lord_in_rasi]))
    else:
        lord_of_ninth_in_navamsa = ninth_lord_in_rasi
    vy2 = _planet_house(p_to_h, lord_of_ninth_in_navamsa) == (asc_house+1)%12
    return vy1 and vy2
def siva_yoga_from_planet_positions(planet_positions):
    ...
    ninth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+8)%12))
    tenth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+9)%12))
    sy = _planet_house(p_to_h, fifth_lord) == (asc_house+8)%12 and _planet_house(p_to_h, ninth_lord) == (asc_house+9)%12 and _planet_house(p_to_h, tenth_lord) == (asc_house+4)%12
    return sy
...
def gouri_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    asc_house_navamsa = globals().get('p_to_h_navamsa', {}).get(const._ascendant_symbol, asc_house)
    navamsa_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,asc_house_navamsa))
    rasi_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,asc_house))
    gy = _planet_house(p_to_h, rasi_lord) == (asc_house+9)%12 and _planet_house(p_to_h, navamsa_lord) == (asc_house+9)%12 and _hs(navamsa_lord, (asc_house+9)%12) > const._FRIEND 
    return gy
def chandikaa_yoga_from_planet_positions(planet_positions):
    ...
    navamsa_map = globals().get('p_to_h_navamsa', {})
    if sixth_lord in navamsa_map:
        sixth_lord_owner_in_navamsa = _as_int(house.house_owner_from_planet_positions(planet_positions, navamsa_map[sixth_lord]))
    else:
        sixth_lord_owner_in_navamsa = sixth_lord
    if ninth_lord in navamsa_map:
        ninth_lord_owner_in_navamsa = _as_int(house.house_owner_from_planet_positions(planet_positions, navamsa_map[ninth_lord]))
    else:
        ninth_lord_owner_in_navamsa = ninth_lord
    cy2 = _planet_house(p_to_h,0) == _planet_house(p_to_h, sixth_lord_owner_in_navamsa) and _planet_house(p_to_h,0) == _planet_house(p_to_h, ninth_lord_owner_in_navamsa)
    return cy1 and cy2
def lakshmi_yoga_from_planet_positions(planet_positions):
    ...
    ninth_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,(asc_house+8)%12))
    lagna_lord = _as_int(house.house_owner_from_planet_positions(planet_positions,asc_house))
    ly1 = _hs(ninth_lord, _planet_house(p_to_h, ninth_lord)) > const._FRIEND and _planet_house(p_to_h, ninth_lord) in house.quadrants_of_the_raasi(asc_house)
    ly2 =  _hs(lagna_lord, _planet_house(p_to_h, lagna_lord)) > const._FRIEND
    return ly1 and ly2
def saarada_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    tenth_lord = _owner(planet_positions,(asc_house+9)%12)
    sy1 = _planet_house(p_to_h, tenth_lord)==(asc_house+4)%12
    if not sy1:
        return False
    sy2 = p_to_h[3] in quadrants_of_the_house(asc_house)
    if not sy2:
        return False
    sy3 = p_to_h[0]==4 and _hs(0,4) > const._FRIEND
    if not sy3:
        return False
    ...
    # BUGFIX: function previously returned None; return composite condition
    return sy1 and sy2 and sy3 and any(sy4) and sy5
def bhaarathi_yoga_from_planet_positions(planet_positions):
    ...
    navamsa_map = globals().get('p_to_h_navamsa', {})
    navamsa_lords = []
    for lord in [second_lord, fifth_lord, eleventh_lord]:
        if lord in navamsa_map:
            nav_lord = _as_int(house.house_owner_from_planet_positions(planet_positions, navamsa_map[lord]))
        else:
            nav_lord = lord
        navamsa_lords.append(nav_lord)
    by_checks = [_planet_house(p_to_h, ninth_lord) == _planet_house(p_to_h, nl) and _hs(nl, _planet_house(p_to_h, ninth_lord)) > const._FRIEND for nl in navamsa_lords]
    # BUGFIX: list returned; should be boolean if any navamsa lord satisfies condition
    return any(by_checks)
def saraswathi_yoga_from_planet_positions(planet_positions):
    ...
    if not sy1:
        return False
    sy2 = _hs(4, _planet_house(p_to_h,4)) > const._NEUTRAL_SAMAM
    return sy1 and sy2
def amsaavatara_yoga_from_planet_positions(planet_positions):
    ...
    ay2 = _hs(6, _planet_house(p_to_h,6)) > const._FRIEND
    return ay1 and ay2
def devendra_yoga_from_planet_positions(planet_positions):
    ...
    if not dy1:
        return False
    second_lord = _owner(planet_positions,(asc_house+1)%12)
    tenth_lord = _owner(planet_positions,(asc_house+9)%12)
    dy2 = _planet_house(p_to_h, second_lord) == (asc_house+9)%12 and _planet_house(p_to_h, tenth_lord) == (asc_house+1)%12
    if not dy2:
        return False
    lagna_lord = _owner(planet_positions,asc_house)
    eleventh_lord = _owner(planet_positions,(asc_house+10)%12)
    dy3 = _planet_house(p_to_h, lagna_lord) == (asc_house+10)%12 and _planet_house(p_to_h, eleventh_lord) == asc_house
...
def indra_yoga_from_planet_positions(planet_positions):
    ...
    if not iy1:
        return False
    fifth_lord = _owner(planet_positions,(asc_house+4)%12)
    eleventh_lord = _owner(planet_positions,(asc_house+10)%12)
    iy2 = _planet_house(p_to_h, fifth_lord) == (asc_house+10)%12 and _planet_house(p_to_h, eleventh_lord) == (asc_house+4)%12
    return iy1 and iy2
def ravi_yoga_from_planet_positions(planet_positions):
    ...
    if not ry1:
        return False
    tenth_lord = _owner(planet_positions,(asc_house+9)%12)
    ry2 = _planet_house(p_to_h, tenth_lord) == (asc_house+2)%12 and p_to_h[6] == (asc_house+2)%12
    return ry1 and ry2 
...
def gandharva_yoga_from_planet_positions(planet_positions):
    ...
    lagna_lord = _owner(planet_positions,asc_house)
    tenth_lord = _owner(planet_positions,(asc_house+9)%12)
    gy1 = _planet_house(p_to_h, tenth_lord) in house.trines_of_the_raasi((asc_house+6)%12)
    if not gy1:
        return False
    #gy2 = p_to_h[lagna_lord] == p_to_h[4] or p_to_h[lagna_lord] in house.aspected_rasis_of_the_planet(h_to_p, 4)
    gy2 = _planet_house(p_to_h, lagna_lord) == p_to_h[4] or str(lagna_lord) in house.graha_drishti_of_the_planet(h_to_p, 4)
    #print('gy2',p_to_h[lagna_lord],p_to_h[4],str(lagna_lord),house.graha_drishti_of_the_planet(h_to_p, 4))
    if not gy2:
        return False
    gy3 = _hs(0, _planet_house(p_to_h,0)) > const._FRIEND
    if not gy3:
        return False
...
def go_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    gy1 = _hs(4, const.moola_trikona_of_planets[4]) > const._FRIEND
    if not gy1:
        return False
    second_lord = _owner(planet_positions,(asc_house+1)%12)
    gy2 = p_to_h[4] == _planet_house(p_to_h, second_lord)
    if not gy2:
        return False
    lagna_lord = _owner(planet_positions,asc_house)
    gy3 = _hs(lagna_lord, _planet_house(p_to_h, lagna_lord)) > const._FRIEND
...
def vidyut_yoga_from_planet_positions(planet_positions):
    ...
    eleventh_lord = _owner(planet_positions,(asc_house+10)%12)
    lagna_lord = _owner(planet_positions,asc_house)
    vy1 = _hs(eleventh_lord, _planet_house(p_to_h, eleventh_lord)) > const._FRIEND
    if not vy1:
        return False
    vy2 = _planet_house(p_to_h, eleventh_lord) == p_to_h[5]
    if not vy2:
        return False
    vy3 = _planet_house(p_to_h, eleventh_lord) in house.quadrants_of_the_raasi(_planet_house(p_to_h, lagna_lord))
...
def chapa_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    fourth_lord = _owner(planet_positions,(asc_house+3)%12)
    tenth_lord = _owner(planet_positions,(asc_house+9)%12)
    cy1 = _planet_house(p_to_h, fourth_lord) == (asc_house+9)%12 and _planet_house(p_to_h, tenth_lord) == (asc_house+3)%12
    lagna_lord = _owner(planet_positions,asc_house)
    cy2 = _hs(lagna_lord, _planet_house(p_to_h, lagna_lord)) > const._FRIEND
    return cy1 and cy2
def pushkala_yoga_from_planet_positions(planet_positions):
    ...
    moon_house = p_to_h[1]
    lagna_lord = _owner(planet_positions,asc_house)
    py1 = _planet_house(p_to_h, lagna_lord) == moon_house
    dispositor_of_moon = _owner(planet_positions,moon_house)
    dispositor_house = _planet_house(p_to_h, dispositor_of_moon)
    ...
    py2_3 = str(dispositor_of_moon) in house.graha_drishti_of_the_planet(h_to_p,asc_house)
...
def makuta_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    ninth_lord = _owner(planet_positions,(asc_house+8)%12)
    my1 = p_to_h[4] == (_planet_house(p_to_h, ninth_lord)+8)%12
...
def jaya_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    tenth_lord = _owner(planet_positions,(asc_house+9)%12)
    jy1 = _hs(tenth_lord, _planet_house(p_to_h, tenth_lord)) == const._EXALTED_UCCHAM
    sixth_lord = _owner(planet_positions,(asc_house+5)%12)
    jy2 = _hs(sixth_lord, _planet_house(p_to_h, sixth_lord)) == const._DEFIBILATED_NEECHAM
    return jy1 and jy2
def harsha_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    sixth_lord = _owner(planet_positions,(asc_house+5)%12)
    return _planet_house(p_to_h, sixth_lord) == (asc_house+5)%12
def sarala_yoga_from_planet_positions(planet_positions):
    h_to_p = utils.get_house_planet_list_from_planet_positions(planet_positions)
    """ Sarala Yoga: If the 8th lord occupies the 8th house """
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    eighth_lord = _owner(planet_positions,(asc_house+7)%12)
    return _planet_house(p_to_h, eighth_lord) == (asc_house+7)%12
def vimala_yoga_from_planet_positions(planet_positions):
    h_to_p = utils.get_house_planet_list_from_planet_positions(planet_positions)
    """ Vimala Yoga: If the 12th lord occupies the 12th house """
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    twelfth_lord = _owner(planet_positions,(asc_house+11)%12)
    return _planet_house(p_to_h, twelfth_lord) == (asc_house+11)%12

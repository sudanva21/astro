...
try:
    ...
    from PyQt6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QCompleter, \
                                QMessageBox, QComboBox, QPushButton, QApplication, QFileDialog, QTreeWidget, QTreeWidgetItem  # type: ignore
    from PyQt6.QtCore import Qt  # type: ignore
except Exception:  # pragma: no cover - headless/test environment fallback
    _PYQT_AVAILABLE = False
    class QWidget:  # minimal stubs for tests that only import formatting helpers
        pass
    class QTreeWidget: pass
    class QTreeWidgetItem:  # noqa: D401
...
"""
Helpers for formatting Vimsottari chain strings for the desktop UI.
These mirror the compact labels used in the web frontend: MD/AD/PD/SD/PAD
and short planet abbreviations.
"""
# Level short labels for vimsottari chain positions
_VIM_LEVEL_LABELS = ['MD','AD','PD','SD','PAD']
# Fallback short names for common planet tokens (lower-case keys)
_PLANET_SHORT_MAP = {
...
def _short_planet(token: str) -> str:
    """Return a short, human-friendly planet abbreviation for a token like 'Mars♂' or 'Raagu☊'."""
    if not token:
        return ''
...
def _format_vimsottari_chain(chain_str: str) -> str:
    """Format a hyphen-separated vimsottari chain into a compact labeled string.

    Example: 'Mars♂-Mars♂-Jupiter♃' -> 'MD:Ma → AD:Ma → PD:Ju'
    """
...
class ChartSimple(QWidget):
    ...
    # Right-side info area: label + expandable Vimsottari tree
    ...
    self._info_label2.setFixedHeight(int(_info_label2_height/6))
    # container widget for label + tree
    self._info_panel2 = QWidget()
    v2 = QVBoxLayout()
    v2.setContentsMargins(0,0,0,0)
    v2.addWidget(self._info_label2)
    # Tree to show per-mahadasha expandable chains
    self._vimsottari_tree = QTreeWidget()
    self._vimsottari_tree.setColumnCount(2)
    self._vimsottari_tree.setHeaderLabels(["Period","Ends"])
    self._vimsottari_tree.setRootIsDecorated(True)
    self._vimsottari_tree.setFixedHeight(int(_info_label2_height*0.9))
    v2.addWidget(self._vimsottari_tree)
    self._info_panel2.setLayout(v2)
    h_layout.addWidget(self._info_panel2)
    self._v_layout.addLayout(h_layout)
    ...
    def compute_horoscope(self):
        ...
        # Request full Vimsottari including nested antardhasa/antara levels (matches original behavior)
        self._vimsottari_dhasa_bhukthi_info = self._horo._get_vimsottari_dhasa_bhukthi(_dob,_tob,_place, include_antardhasa=True)
    ...
    def _fill_information_label2(self,format_str):
        info_str = ''
        # dob, tob, place variables were previously collected but not used here; removed to avoid linter warnings
        ...
        for p,(k,v) in enumerate(_vimsottari_dhasa_bhukti_info):
            ...
            # Get dhasa end Date
                ...
                di += 9
        # Add a compact summary of the Vimsottari chains (MD/AD/PD/...) for desktop display
        try:
            if _vimsottari_dhasa_bhukti_info:
                mah_list = []
                summaries = []
                full_chains = []
                # Build Mahadasha grouped structure for tree widget: iterate and collect children
                groups = []
                current_group = None
                for item in _vimsottari_dhasa_bhukti_info:
                    # item may be (chain, date) or (k,v,...) depending on source; handle tuple shapes
                    if isinstance(item, (list, tuple)) and len(item) >= 2:
                        ...
                        v = ''
                    mah = k.split('-')[0]
                    if current_group is None or current_group['mah'] != mah:
                        current_group = {'mah': mah, 'items': []}
                        groups.append(current_group)
                    current_group['items'].append((k, v))
                # Populate QTreeWidget groups
                self._vimsottari_tree.clear()
                for g in groups:
                    mah_item = QTreeWidgetItem(self._vimsottari_tree, [g['mah'], ''])
                    mah_item.setExpanded(False)
                    for k,v in g['items']:
                        compact = _format_vimsottari_chain(k)
                        child = QTreeWidgetItem(mah_item, [compact, v])
                        mah_item.addChild(child)
                        full_chains.append(k)
                if summaries:
                    key = 'Vimsottari'
                    value = '; '.join(summaries)
                    info_str += format_str % (key, value)
                    # set tooltip on the info label with full chains
                    try:
                        self._info_label2.setToolTip('\n'.join(full_chains))
                    except Exception:
                        pass
        except Exception:
            # silently ignore formatting errors to avoid breaking UI
            pass

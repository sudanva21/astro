...
class ChartTabbed(QWidget):
    ...
    def _update_combos(self):
        ...
        if not self._western_chart:
            ...
            self._dhasa_combo.clear()
            if self._dhasa_type_combo.currentIndex()==0:
                # Show only Vimsottari-family graha dhasas in the Nakshatra/Graha dhasa combo to avoid
                # presenting every available graha dhasa and causing confusion where "all dashas"
                # appear together. This keeps the combo focused on vimsottari variants.
                vimsottari_keys = [d for d in _graha_dhasa_dict.keys() if 'vimsottari' in d]
                # Fall back to full list if none found (defensive)
                items = vimsottari_keys if len(vimsottari_keys) > 0 else list(_graha_dhasa_dict.keys())
                self._dhasa_combo.addItems([self.resources[d+'_str'] for d in items])
                self._dhasa_options_button.setVisible(True); self._dhasa_varga_combo.setVisible(False)
            elif self._dhasa_type_combo.currentIndex()==1:
                self._dhasa_options_button.setVisible(False); self._dhasa_varga_combo.setVisible(True)
    ...
    def compute_horoscope(self, calculation_type='drik'):
        ...
        if not self._western_chart:
            ...
            # Only precompute Vimsottari-family graha dhasas here to avoid populating the UI with
            # every graha dhasa result which can be confusing (looks like "all dashas").
            for tab_str,tab_values in _graha_dhasa_dict.items():
                if 'vimsottari' not in tab_str:
                    tab_values[7] = []
                    continue
                option_str = ''
